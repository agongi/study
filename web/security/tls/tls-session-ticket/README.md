...
- First TLS-handshake request initiated sending "Client Hello" along with **empty Session Ticket**
- The server will answer with an `New Session Ticket` extension in its "Server Hello" message if it supports it
...
- The client should store it and present it in the “Client Hello” message of the next session
  - this state is encrypted and integrity-protected by a key known only by the server.
- The server finds the corresponding session in its cache and accepts to resume the session, it will send back the same session identifier and will continue with the abbreviated TLS-handshake
