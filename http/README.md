...
**Keep-alive / Persistent Connections**<br>
...
Each creation of connection needs `RTT` to make channel.
...
> Pipelining enables client to send all request before response. Server responses packet `in the same order` that the requests were received.

> It has [Head-of-line blocking](https://en.wikipedia.org/wiki/Head-of-line_blocking) problem. (Output is occupied by first packet in line.)

> It is rarely used due to difficulty of implementation.

<img src="https://github.com/agongi/study/blob/master/http/images/Screen%20Shot%202016-02-01%20at%2023.36.18.png" width="70%">
...
**HTTPS only**<br>
SPDY only supports over `HTTPS`.

**One-TCP-Connection**<br>
...
> Multiplexing enables client to send all request before response. Server responses packet out-of-order that the requests were received.

> Priority bit can be sent in request from client and server is able to send response `out-of-order` based on request priority.

![alt-http-multiplexing](https://github.com/agongi/study/blob/master/http/images/Zp2lf.png)

**Server Push**<br>
Server can push static resources e.g. css and/or javascript while processing HTML.<br>
Pushed resources are cached in browser and `cache-hit (HTTP 304 Not Modifed)` when request it.

<img src="https://github.com/agongi/study/blob/master/http/images/Screen%20Shot%202016-02-02%20at%2000.34.17.png" width="50%">

**Header Compression**<br>
HTTP header is compressed. It significantly reduces HTTP response size by compression and improves performance almost 30%.
